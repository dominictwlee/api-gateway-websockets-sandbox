...
class Client {
  ...
  async _setupClient(config) {
    ...
    if (typeof config !== 'object' && !this.client) {
      ...
      console.log(`[wsClient][_setupClient][Table Get] Item: \n${JSON.stringify(item, null, 2)}`);
    ...
    if (!this.client) {
      if (config.requestContext.apiId) {
        config.requestContext.domainName = `${config.requestContext.apiId}.execute-api.${
  ...
  async send(connection, payload) {
    ...
    console.log(`[wsClient][send] Connection: ${connection}`);
    console.log(`[wsClient][send] Payload: \n${JSON.stringify(payload, null, 2)}`);

    // client is an instance of AWS.ApiGatewayManagementApi
    await this.client
      .postToConnection({ ConnectionId, Data: JSON.stringify(payload) })
      .promise()
      .catch(async err => {
        ...
        if (err.statusCode === 410) {
          ...
          console.log(
            `[wsClient][send][postToConnection] Found stale connection, deleting ${ConnectionId}:`
          );
          ...
          const unsubscribes = subscriptions.map(async subscription =>
            db.Client.delete({
              TableName: db.Table,
              Key: {
                [db.Channel.Connections.Key]: `${db.Channel.Prefix}${db.parseEntityId(
